...
def test_stub_provider_passthrough_hotwords_and_language_hint() -> None:
    provider = StubASRProvider()
    request = ASRRequest(
    ...
    assert result.metadata["provider_kind"] == "test_double"
...
def test_stub_provider_defaults_language_to_und() -> None:
    provider = StubASRProvider(default_texts=("hello",))
