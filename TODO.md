...
**已实现**:
- job 页删除自动重试期间显示“自动重试第 N 次”内联提示
- 达到自动重试上限后给出可操作提示（手动再次点击删除任务）
- 文案已接入 i18n（中英文）
...
**已实现**:
- 后端日志文案统一为“阶段 + 动作/结果 + 原因 + 建议”风格（含阶段开始/完成/失败、关键帧异常回退、派发失败）
- 派发失败日志补充 dispatch 阶段上下文与重试建议
- 前端日志栏支持级别标签（info/warning/error/unknown）与连续同级日志分组显示
- 新增日志级别标签 i18n（中英文），并保留原始日志行可追溯
